...
try {
    ...
    $allowed = ['basic','extended','complete'];
    ...
    $item = ['id' => $baseItem['id'], 'label' => $label];

    if (in_array($scope, ['basic','extended','complete'], true)) {
        $item['code'] = $baseItem['code'];
        $item['area'] = $baseItem['area'];
        $item['year'] = $baseItem['year'];
    }

    if (in_array($scope, ['extended','complete'], true)) {
